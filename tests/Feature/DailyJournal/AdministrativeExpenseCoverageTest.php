...
class AdministrativeExpenseCoverageTest extends DailyJournalFeatureTestCase
{
    ...
    /** 16. Zero income and zero expense day: real inventory expense is recorded/uncovered but not deducted from carried surplus. */
    public function test_zero_income_and_expense_day_does_not_deduct_carried_surplus(): void
    {
        $this->actAsFinanceUser();
        $owner = $this->subjectProject();
        $beneficiary = $this->subjectProject(['name' => 'B16 '.uniqid()]);

        // Day one: build up real fund surplus with no administrative expense.
        $dayOne = $this->saveJournal(self::DAY_ONE, $beneficiary->id, ['daily_income' => 1000]);
        $this->assertEqualsWithDelta(880.0, (float) $dayOne->fund_balance, 0.001);

        // Day two: both income and expense are zero, but a real inventory administrative
        // cost exists for this project on this date.
        $this->postAdministrativeOutgoing($owner, $beneficiary, 100, 1, self::DAY_TWO);
        $dayTwo = $this->saveJournal(self::DAY_TWO, $beneficiary->id, [
            'daily_income' => 0,
            'daily_expense' => 0,
        ]);

        $this->assertEqualsWithDelta(100.0, (float) $dayTwo->administrative_expense, 0.001);
        $this->assertEqualsWithDelta(100.0, (float) $dayTwo->uncovered_administrative_expense, 0.001);
        $this->assertEqualsWithDelta(0.0, (float) $dayTwo->project_administration_settled, 0.001);
        $this->assertEqualsWithDelta(100.0, (float) $dayTwo->outstanding_project_administration, 0.001);
        // Fund balance stays exactly at Day 1's surplus — nothing is deducted on a zero-activity day.
        $this->assertEqualsWithDelta(880.0, (float) $dayTwo->fund_balance, 0.001);
        $this->assertEqualsWithDelta(0.0, (float) $dayTwo->administrative_debt, 0.001);

        $day = $this->adminFundDay(self::DAY_TWO);
        $this->assertRoundedMoneyEquals(100.0, $day['project_administration']);
    }

    /** 17. Fixed-type operational deduction is suppressed on a zero-income/zero-expense day. */
    public function test_fixed_operational_deduction_suppressed_on_zero_activity_day(): void
    {
        $this->actAsFinanceUser();
        ...
        $zeroActivity = $this->saveJournal(self::DAY_ONE, $project->id, [
            'daily_income' => 0,
            'daily_expense' => 0,
        ]);
        $this->assertEqualsWithDelta(0.0, (float) $zeroActivity->operational_deduction, 0.001);
        $this->assertEqualsWithDelta(0.0, (float) $zeroActivity->fund_balance, 0.001);

        // Regression: the flat deduction must still apply once there is real activity.
        $withExpense = $this->saveJournal(self::DAY_TWO, $project->id, [
            'daily_income' => 0,
            'daily_expense' => 20,
        ]);
        $this->assertEqualsWithDelta(150.0, (float) $withExpense->operational_deduction, 0.001);
        $this->assertEqualsWithDelta(-170.0, (float) $withExpense->fund_balance, 0.001);
    }
}
